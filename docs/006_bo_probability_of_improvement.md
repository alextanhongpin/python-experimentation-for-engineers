...
# Define the acquisition function (Probability of Improvement)
...
def bayesian_optimization(n_iters, sample_loss, bounds, n_pre_samples=5):
    ...
    for i in range(n_iters):
        ...
        X_next = propose_location(
            probability_of_improvement, X_sample, Y_sample, gpr, bounds
        )
...
def plot_acquisition_function(
    ...
    Y = simple_function(X)
    mu, sigma = gpr.predict(X, return_std=True)
    acquisition = probability_of_improvement(X, X_sample, Y_sample, gpr, xi)
    ...
    plt.plot(X, acquisition, "g--", label="Acquisition Function (PI)")
    plt.axvline(X_next)
    ...
    if i == 0:
        plt.legend()
        plt.title(
            "Bayesian Optimization with Probability of Improvement on Simple Function"
        )
...
![png](006_bo_probability_of_improvement_files/006_bo_probability_of_improvement_0_0.png)
    



    
![png](006_bo_probability_of_improvement_files/006_bo_probability_of_improvement_0_1.png)
    


    Best value found: y = 2.625622024775447e-06 at x = 1.9983796228757553
